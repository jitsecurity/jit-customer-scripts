...
from src.shared.models import Organization, TeamTemplate
...
def organization():
    return Organization(
        teams=[
            TeamTemplate(name="team1", members=[], resources=[]),
            TeamTemplate(name="team2", members=[], resources=[]),
        ]
    )
...
def test_process_teams(organization):
    ...
    with patch("src.scripts.create_teams.get_existing_teams") as mock_get_existing_teams:
        mock_get_existing_teams.return_value = [
            TeamTemplate(name="team1", members=[], resources=[]),
            TeamTemplate(name="team3", members=[], resources=[]),
        ]
        result = process_teams("token", organization)
        assert len(result) == 1
