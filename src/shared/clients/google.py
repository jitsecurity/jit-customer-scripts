...
def get_teams_from_bigquery_view() -> Organization:
    try:
        num_repos = 0
        logger.info(f"Retrieving teams from BigQuery View: {os.getenv('BIGQUERY_VIEW_NAME')}")
        ...
        for row in rows:
            resources = [Resource(type=ResourceType.GithubRepo, name=repo) for repo in row.repos]
            num_repos += len(resources)
            teams[row.ownership_team_name] = TeamStructure(name=row.ownership_team_name, members=[], resources=resources, slack_channel=row.slack_alerting_channel)
        logger.info(f"Retrieved ({len(teams.keys())}) teams {list(teams.keys())} with {num_repos} repos from Google BigQuery successfully.")
